...
while True :
    ...
    names = ['Geonho', 'Hyun', 'Jinseok', 'Myungjae', 'Sangwoo', 'Yechan']
    ...
    for r in results :
        ...
        if not r_b.cls == None :
            ...
            for idx in range(len(r_b)) :
                ...
                if int(r_b.cls[idx]) in list(range(6)) :
                    if r_b.conf[idx] > 0.3 :
                        color = (0,255,0)
                        conf = r_b.conf[idx]*100
                        name = names[int(r_b.cls[idx])]
                        label_text = f'{name} conf-score: {conf:.2f}'
                        print(r_b)
