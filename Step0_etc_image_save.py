...
def capture_owner_images(num_images=170) : ## num_images에 숫자를 입력한만큼 이미지 저장
    ## 0은 기본 웹캠
    ...
    while count < num_images :
        ...
        for (x, y, w, h) in faces :
            ...
            ## 프레임에서 얼굴 영역을 가져오되, 약간의 배경 추가
            ## 범위가 사진 밖을 벗어나는 경우를 방지하기 위해 조정
            # face = frame[y-15:y+h+15, x-15:x+w+15]
            face = frame[:]
            ## 프레임에서 얼굴 영역만 가져온다
            # face = frame[y:y+h, x:x+w]
            ## 탐지한 얼굴 영역 사이즈 변환 : 괄호를 채우면 됩니다.
            resized_face = cv2.resize(face, (512 , 384) )
            ## 파일 이름 설정 및 저장
            face_file = folder_path + f"/jh_face_{count}.jpg"
